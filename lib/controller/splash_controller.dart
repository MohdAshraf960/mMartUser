...
Future<void> getModules({Map<String, String> headers}) async {
    ...
    http.Response response = await http.get(
        Uri.parse("https://admin.madinahmart.in/api/v1/module"),
        headers: {});
    //splashRepo.getModules(headers: headers);
    print("response model------------------> ${response.body}");
    print("response model------------------> ${response.statusCode}");
    if (response.statusCode == 200) {
      ...
      print("_moduleList------------------> ${_moduleList}");
    } else {
      print("else------------------> ${_moduleList}");
      //ApiChecker.checkApi(response);
    }
    update();
    print("_moduleList------------------> ${_moduleList}");
